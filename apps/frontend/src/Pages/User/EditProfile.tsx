  ...
  const fetchUserProfile = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        two_factor_enabled: userData.twofa_required || false,
      });
    } catch (err) {
        ...
        <div className="p-8">
          {loading ? (
            ...
            <>
              ...
              <div className="mb-8">
                {formData.two_factor_enabled ?( <button
                  className="w-full p-3 text-white bg-green-500 rounded hover:opacity-85 focus:outline-none disabled:opacity-50 font-semibold transition-opacity mt-5"
                  disabled={true}
                >
                  Two-Factor Authentication Enabled
                </button>):(userId && <TwoFASetup userId={userId} />) }
              </div>
            </>
          )}
        </div>
